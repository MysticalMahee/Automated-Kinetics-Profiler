...
<ul>
    <li>
        ...
        <strong>Purpose:</strong> Filters out any negative concentrations (hence why the 51st value in the reaction_kinetics.values file is not present). At the tail end of the reaction, simulated instrument noise can push near-zero concentration values below zero. Attempting to pass negative numbers into a natural log function <p>ln(x)</p> throws a <code>NaN</code> error and crashes the regression model. 
